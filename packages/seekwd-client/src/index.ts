...
export function createLocalHostClient(): HostClient {
  ...
  let state: HostSnapshot = stored
    ? { snapshotVersion: stored.snapshotVersion ?? 1, workspaces: stored.workspaces ?? [], canvases: stored.canvases ?? [], nodes: stored.nodes ?? [], revisions: stored.revisions ?? [], runs: (stored.runs ?? []).map((run) => ({ ...run, finishedAt: run.finishedAt ?? null })), notifications: (stored.notifications ?? []).map((notification) => ({ ...notification, runId: notification.runId ?? null, canvasId: notification.canvasId ?? null })) }
    : seed();
