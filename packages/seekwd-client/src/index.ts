import type { Canvas, CanvasEdge, CanvasNode, CanvasRevision, CreateCanvasInput, CreateEdgeInput, CreateNodeInput, CreateWorkspaceInput, HostError, HostEventBatch, HostSnapshot, Id, Notification, Run, StartRunInput, Workspace } from "@seekwd/protocol-schema";
...
const seed = (): HostSnapshot => {
  ...
  return { snapshotVersion: 1, workspaces: [workspace], canvases: [canvas], nodes: [node], edges: [], revisions: [{ id: "rev_citation_3", canvasId: canvas.id, revision: 3, createdAt: now(), createdBy: "user", status: "validated", contentDigest: "sha256:" + "0".repeat(64), graphJson: JSON.stringify({ nodes: [node], edges: [] }) }], runs: [], notifications: [] };
};
...
export function createLocalHostClient(): HostClient {
  ...
  let state: HostSnapshot = stored
    ? { snapshotVersion: stored.snapshotVersion ?? 1, workspaces: stored.workspaces ?? [], canvases: (stored.canvases ?? []).map((canvas) => ({ ...canvas, draftRevision: canvas.draftRevision ?? 0, draftDirty: canvas.draftDirty ?? false })), nodes: stored.nodes ?? [], edges: stored.edges ?? [], revisions: (stored.revisions ?? []).map((revision) => ({ ...revision, contentDigest: revision.contentDigest ?? "sha256:" + "0".repeat(64), graphJson: revision.graphJson ?? JSON.stringify({ nodes: [], edges: [] }) })), runs: (stored.runs ?? []).map((run) => ({ ...run, finishedAt: run.finishedAt ?? null })), notifications: (stored.notifications ?? []).map((notification) => ({ ...notification, runId: notification.runId ?? null, canvasId: notification.canvasId ?? null })) }
    : seed();
  ...
  const client: HostClient = {
    ...
    async createNode(input) { const item = canvas(input.canvasId); if (!item) throw new Error("Canvas not found"); const node: CanvasNode = { id: uid("node"), canvasId: item.id, name: input.name.trim() || "Untitled node", kind: input.kind.trim() || "task.manual" }; item.draftRevision += 1; item.draftDirty = true; item.updatedAt = now(); state.nodes.push(node); commit(); return structuredClone(node); },
    async createEdge(input) { const item = canvas(input.canvasId); if (!item) throw new Error("Canvas not found"); if (input.sourceNodeId === input.targetNodeId) throw new Error("A node cannot connect to itself"); if (!state.nodes.some((node) => node.id === input.sourceNodeId && node.canvasId === item.id) || !state.nodes.some((node) => node.id === input.targetNodeId && node.canvasId === item.id)) throw new Error("Both edge endpoints must belong to the canvas"); if (state.edges.some((edge) => edge.canvasId === item.id && edge.sourceNodeId === input.sourceNodeId && edge.targetNodeId === input.targetNodeId)) throw new Error("This edge already exists"); const edge: CanvasEdge = { id: uid("edge"), canvasId: item.id, sourceNodeId: input.sourceNodeId, targetNodeId: input.targetNodeId }; item.draftRevision += 1; item.draftDirty = true; item.updatedAt = now(); state.edges.push(edge); commit(); return structuredClone(edge); },
...
export function createHttpHostClient(baseUrl = "http://127.0.0.1:4317"): HostClient {
  ...
  const client: HostClient = {
    ...
    createEdge: (input) => request<CanvasEdge>(`/api/canvases/${input.canvasId}/edges`, { method: "POST", body: JSON.stringify({ sourceNodeId: input.sourceNodeId, targetNodeId: input.targetNodeId }) }),
