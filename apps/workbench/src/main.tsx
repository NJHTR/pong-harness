...
function App() {
  ...
  const run = async () => { if (!activeCanvas) return; try { await client.startRun({ canvasId: activeCanvas.id, revision: activeCanvas.revision, entrypoint: "default", idempotencyKey: crypto.randomUUID() }); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to start run"); } };
  const createWorkspace = async () => { const item = await client.createWorkspace({ name: "New Workspace", path: "D:/Documents/New Workspace" }); setWorkspaceId(item.id); setExpanded((state) => ({ ...state, [item.id]: true })); };
