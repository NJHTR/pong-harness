...
- Host 提供本机 HTTP 的 Workspace/Canvas 创建与改名、默认入口设置、保存简化 Revision、启动/查询模拟 Run、快照与游标查询；开发接口要求启动 token、固定 Host authority 和显式允许的 Origin，Workbench 通过 Vite 同源开发代理连接。
...
- 无 Tauri 桌面壳和安装包；Host 当前只有开发用 Bearer token/Origin 边界，尚无本机用户绑定 Session、受保护 IPC、单实例锁与完整进程生命周期，仍不能承载任意本地执行。Vite 代理不是正式发行安全边界。
