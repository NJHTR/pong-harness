...
| HST-C-002 | Host | SQLite 初始化 | WAL、版本迁移、事务、外键、索引和恢复检查。 | 0% | W2 | BE/OPS | 已完成首个可验证切片：文件库 WAL、busy timeout、`user_version=1`、必需表/索引/约束校验，0 版旧表事务迁移、部分库/高版本拒绝启动和外键检查；对应证据见需求台账与 `011f9a0`。仍缺备份/恢复点、真实崩溃注入和完整 Event Store，历史百分比不代表当前验收率。 |
