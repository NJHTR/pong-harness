...
- 当前 Canvas 的 `Run`、`Add node`、`Run History` 和 `Run Output` 都属于画布内部工具栏。`Run` 仅在画布存在且只有一个 `primary` entry node 时显示；无主节点时画布处于不可运行状态，并在配置面板提示原因。
- 运行成功、失败、需要审批或等待输入时，使用工作台通知提供主动反馈；通知只说明结果和下一步入口，不替代画布内的详细运行记录。

### 主节点与连接点

- 一个 Canvas 必须且只能有一个 `primary` entry node。节点 ID 是稳定身份，设为主节点是原子操作，会自动取消旧主节点；删除主节点后 Run 隐藏，直到用户指定新的主节点。
- 主节点负责定义画布执行入口和初始上下文，不等同于普通数据节点。画布可以有任意数量的普通节点、触发器和终止节点。
