...
> 规范优先级：完整页面和状态覆盖见 [02-information-architecture.md](02-information-architecture.md)；身份、版本、入口、运行状态、GraphPatch、权限和执行语义以 [27-normative-contracts/](../27-normative-contracts/00-README.md) 为准。本文中的旧称仅用于 UI 兼容说明，不得作为跨组件协议。
...
- 每个 Canvas 行右侧显示该 Canvas 的运行态图标；`error`、`waiting`、`running`、`success`、`idle` 必须使用语义 SVG，并在 Workspace 折叠时聚合为最高优先级状态。状态不能只存在于当前画布。
- 当前 Canvas 的 `Run`、`Add node`、`Run History` 和 `Run Output` 都属于画布内部工具栏。`Run` 使用默认入口；`Run from...` 用于选择命名入口。只有画布已经保存为 Revision、入口允许手动调用、输入和策略均满足时才能运行。不存在默认入口或入口未配置时，画布处于不可运行状态，并在配置面板提示原因。
- 运行成功、失败、需要审批或等待输入时，使用工作台通知提供主动反馈；通知只说明结果和下一步入口，不替代画布内的详细运行记录。

### 主节点与连接点

- 一个 Canvas Revision 必须且只能有一个 `defaultEntrypointId`，同时允许多个命名入口和多个事件、定时、Webhook 或画布调用触发器。默认入口可以指向一个普通节点或专用入口节点，但不能代表画布全部入口。
- `primaryNodeId` 仅是旧版本迁移字段，表示默认入口的目标节点；新 UI 不再把它当作画布唯一入口。删除默认入口目标时，必须重新绑定默认入口或将画布标记为不可运行；删除其他命名入口只影响引用它的触发器和调用方。
- 主入口负责定义手动运行的初始上下文，不等同于普通数据节点。画布可以有任意数量的普通节点、触发器和终止节点。
- 端口数据模型支持任意数量的输入和输出、一个端口多条边、跨画布引用和事件/数据混合连接。视觉层默认展示少量端口，超过阈值显示 `N more`，展开仍有上限，完整端口清单进入 Inspector。
- 端口标签必须省略溢出并保留连接计数；连接计数不是装饰状态，而是同一端口有多条边时的可操作摘要。极端数量不得改变节点宽度或覆盖其他节点。
