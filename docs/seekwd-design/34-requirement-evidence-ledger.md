...
| `HST-C-002` | SQLite 快照和事务 (`d1844f7`)，失败回滚 (`82b08e8`)，启动拒绝坏数据 (`f18faf8`)，初始化/迁移边界与文件库验证 (`011f9a0`) | `failed_persist_does_not_advance_snapshot_version`、`failed_candidate_persist_restores_store`、`invalid_snapshot_does_not_become_an_empty_store`、`missing_snapshot_table_is_not_treated_as_an_empty_store`、`snapshot_and_event_cursor_must_agree`、`file_database_initializes_wal_version_pragmas_and_index`、`newer_schema_version_is_rejected_without_downgrade`、`partial_unversioned_database_is_rejected_without_initializing_remaining_tables`、`legacy_unversioned_database_migrates_preserving_rows_and_constraints`；`cargo test --workspace`（24 个 Host 测试）、`cargo check --workspace`、`pnpm check:requirements`、`git diff --check` 通过（2026-09-25） | **部分实现**；已落地文件库 WAL、5 秒 busy timeout、`synchronous=NORMAL`、外键开关、`user_version=1`、必需表/索引/约束启动校验；0 版旧表在事务内迁移并保留数据，部分库和更高版本拒绝启动，持久化事务执行外键检查。仍缺备份/恢复点、迁移回滚演练、真实崩溃注入和正式 Event Store/外键关系。 |
