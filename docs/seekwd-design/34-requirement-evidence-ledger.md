...
> 基线：2026-09-26，`dev` 分支 `bb0a317`；只覆盖已核对的纵向切片，不代表全系统需求均已复核。
...
|---|---|---|---|
| `P0-009` | `packages/protocol-schema/host-wire.schema.json`、生成器与 `src/host-wire.generated.ts`；Rust serde 位于 `crates/pong-core/src/lib.rs` 和 `crates/pong-host/src/main.rs`；节点/边请求与 `HostSnapshot.edges` 在 `6d07169` 冻结 | `pnpm --filter @seekwd/protocol-schema test`（3 个共享样本/负例测试）；`cargo test --workspace`（26 个 Host 测试，含共享快照、节点/边命令和稳定图摘要）；`pnpm -r typecheck`、`pnpm -r build`、`pnpm check:requirements`、`git diff --check` 通过（2026-09-26） | **部分实现**；当前 Host HTTP 切片已有单一 JSON Schema 生成 TS 类型、Rust serde/HTTP 跨语言样本验证和图边字段。完整领域、Rust/SQLite/IPC 生成、版本兼容、JSON extractor 错误归一化及安全整数边界尚未完成。详见 `packages/protocol-schema/README.md`。 |
...
| `HST-C-002` | SQLite 快照和事务 (`d1844f7`)，失败回滚 (`82b08e8`)，启动拒绝坏数据 (`f18faf8`)，初始化/迁移边界与文件库验证 (`011f9a0`)，图节点/边快照持久化 (`6d07169`) | `failed_persist_does_not_advance_snapshot_version`、`failed_candidate_persist_restores_store`、`invalid_snapshot_does_not_become_an_empty_store`、`missing_snapshot_table_is_not_treated_as_an_empty_store`、`snapshot_and_event_cursor_must_agree`、`file_database_initializes_wal_version_pragmas_and_index`、`newer_schema_version_is_rejected_without_downgrade`、`partial_unversioned_database_is_rejected_without_initializing_remaining_tables`、`legacy_unversioned_database_migrates_preserving_rows_and_constraints`、`graph_commands_persist_nodes_edges_and_mark_draft_dirty`；`cargo test --workspace`（26 个 Host 测试）通过（2026-09-26） | **部分实现**；已落地文件库 WAL、5 秒 busy timeout、`synchronous=NORMAL`、外键开关、`user_version=1`、必需表/索引/约束启动校验和最小图快照持久化。仍缺备份/恢复点、迁移回滚演练、真实崩溃注入和正式 Event Store/外键关系。 |
...
| `CVS-C-001` | Host 创建 Canvas 和 `trigger.start` 节点 (`0919ff8`)；Canvas 输出增加 `draftRevision`/`draftDirty`；节点/边写入并标记 Draft dirty (`6d07169`) | `legacy_snapshot_defaults_draft_and_revision_content_fields`、`graph_commands_persist_nodes_edges_and_mark_draft_dirty`；创建 Canvas 初始化 Draft 版本字段 | **部分实现**；当前只有单一默认入口和简化节点集合，尚无完整 GraphDocument、命名入口/触发器或 Draft 命令 API。 |
| `CVS-U-001` | Host 原地更新 Canvas 显示名 (`5ea6683`)；Workbench 有入口 (`acb09c0`) | 无历史 Run/Release 引用测试 | **部分实现**；尚无 Release 和跨画布引用验证。 |
| `REV-C-001` | Host 保存 Revision 编号和元数据 (`5ea6683`)；当前保存冻结按画布排序的节点和边图 JSON、`sha256:` 内容摘要，并支持 `expectedDraftRevision` (`6d07169`) | `save_revision_requires_current_draft_revision_and_freezes_digest`（含节点与边）、`graph_digest_is_stable_for_node_order`（含边顺序）、`legacy_snapshot_defaults_draft_and_revision_content_fields`；Schema fixture 含 `draftRevision`、`draftDirty`、`contentDigest`、`graphJson` | **部分实现**；已建立最小 Draft 乐观版本和 Revision 内容冻结，但还没有完整 GraphDocument、端口、ValidationReport、CompatibilityManifest、Release 或多 Draft。 |
...
| `FNT-DATA-001` | `packages/seekwd-client/src/index.ts` 的 `HostClient`、本地与 HTTP 适配器 (`5b5ee83`、`0b26e04`)；新增 `createNode`/`createEdge` 并统一本地 Draft dirty 语义 (`bb0a317`) | `pnpm -r typecheck`、`pnpm -r build`；无适配器合同测试 | **部分实现**；Workbench 已能通过同一 HostClient 编辑节点/边和保存最小 Revision；协议字段仍缺完整 Command/Query/Event 信封，本地适配器使用 `localStorage`。 |
| `FNT-DATA-004` | HTTP 轮询后重新获取快照 (`0b26e04`、`2f399fd`)，Workbench 动态读取 `HostSnapshot.nodes/edges` 并渲染图 (`bb0a317`) | `pnpm -r typecheck`、`pnpm -r build`；无断线恢复/去重 E2E | **部分实现**；无持久 Cursor、Gap 检测、Event ID 去重及离线事实隔离。 |
